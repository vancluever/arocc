...
fn codegenDiagnostic(p: *Parser, usage_tok: TokenIndex, diagnostic: Diagnostic, args: anytype) !void {
    ...
    var bfa = std.heap.stackFallback(1024, p.comp.gpa);
    var allocating: std.Io.Writer.Allocating = .initAligned(bfa.get(), .of(u32));
    defer allocating.deinit();
...
fn staticAssertEvaluationNote(p: *Parser, node: Node.Index, requirement: []const u8) Error!void {
    ...
    var note_bfa = std.heap.stackFallback(1024, p.comp.gpa);
    var note_allocating: std.Io.Writer.Allocating = .init(note_bfa.get());
...
fn staticAssert(p: *Parser) Error!bool {
    ...
    if (res.val.opt_ref == .none) {
        ...
        if (!res.val.toBool(p.comp)) {
            var bfa = std.heap.stackFallback(1024, gpa);
            var allocating: std.Io.Writer.Allocating = .init(bfa.get());
            defer allocating.deinit();
...
fn gnuAsmStmt(p: *Parser, quals: Tree.GNUAssemblyQualifiers, asm_tok: TokenIndex, l_paren: TokenIndex) Error!Node.Index {
    ...
    var bfa = std.heap.stackFallback(bytes_needed, gpa);
...
fn binaryExpr(p: *Parser, min_prec: Token.Precedence, eval: bool) Error!?Result {
    ...
    while (true) {
        var op_prec = Token.precedence[@intFromEnum(p.tok_ids[p.tok_i])];
